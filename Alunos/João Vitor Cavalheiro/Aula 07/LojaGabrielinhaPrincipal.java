...
public class LojaGabrielinhaPrincipal {
	...
	public static void main(String[] args) {
		...
		Cliente cliente = new Cliente("Leonardo Silva",
				18,
				"Cascavel",
				"Centro",
				"Av:Tancredo Neves");

		Vendedor vendedor = new Vendedor("Claudio",
				31,
				"Cascavel",
				"Centro",
				"Av:Tancredo Neves",
		...
		ProcessaPedido processoPedido = new ProcessaPedido(cliente,
		...
		ProcessaPedido processoPedido2 = new ProcessaPedido(cliente,
				LocalDate.of(2026, 02, 10),
				LocalDate.of(2026, 02, 12),
		...
		Item item1 = new Item(1, "Flor1", "planta", 10);
		Item item2 = new Item(2, "Flor2", "planta", 11);
		Item item3 = new Item(3, "Flor3", "planta", 12);

		item1.gerarDescricao();
		item2.gerarDescricao();
		item3.gerarDescricao();

		processoPedido.AddItem(item3);
		processoPedido.AddItem(item2);
		processoPedido2.AddItem(item1);

		processoPedido.gerarDescricaoVenda();
		processoPedido2.gerarDescricaoVenda();
	...
	private static void criarPedido() {
		...
		System.out.println("===== ADIÇÃO DADOS FAKE =====");
		Cliente cliente = new Cliente("Leonardo Silva",
				18,
				"Cascavel",
				"Centro",
		...
		LocalDate dataPagamento = LocalDate.now().plusDays(1);
		LocalDate dataVencimento = LocalDate.now().plusDays(2);

		ProcessaPedido processador = new ProcessaPedido(cliente,
				dataCriacao,
				dataPagamento,
				dataVencimento,
				id,
		...
		processador.AddItem(item3);
		processador.AddItem(item2);
		processador.AddItem(item1);

		if (processador != null) {
			System.out.println("Pedido criado com sucesso!");
			processador.gerarDescricaoVenda();
		} else {
	...
	private static void validarEscolhaF01(int escolhaF01, int escolhaL) {
		switch (escolhaF01) {
			...
			case 1 -> {
				lojas.get(escolhaL - 1).apresentarLogradouro();
			}
			case 2 -> {
